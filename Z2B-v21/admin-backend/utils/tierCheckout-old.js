...
async function createTierCheckout(req) {
    ...
    // Get settings to validate tier
    const settings = await Settings.findOne();
    if (!settings || !settings.tiers[dbTierCode]) {
        throw new Error('Invalid tier selected');
    }

    const tier = settings.tiers[dbTierCode];

    // Determine price (use betaPrice if beta program is active)
    const isBeta = settings.general && settings.general.betaProgramActive;
    ...
    try {
        ...
        console.warn('⚠️  Could not save payment session to MongoDB:', saveError.message);
